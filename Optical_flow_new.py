...
def calculate_optical_flow(video_path, output_folder):
    ...
    while True:  # Continue processing frames until 64 frames are accumulated
        ret, frame = cap.read()
        if not ret:
            break
        ...
        if frame_count >= 1:  # After processing 1 frame, write the result and reset accumulator
        ...
        if frame_count == 64:
            break  # Stop when 64 frames are accumulated
...
def preprocess_all_videos(input_folder, output_folder):
    for file_name in os.listdir(input_folder):
        if file_name.endswith('_processed.mp4'):
            ...
            # Pass frame_window argument as 64 frames
...
output_video_folder = r"C:\Users\andre\OneDrive\Documents\GitHub\Brainvision_Project\Motion_files\Optical_flow"
